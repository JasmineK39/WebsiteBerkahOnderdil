    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between py-4">
        <router-link to="/" class="flex items-center space-x-3 shrink-0">
          <div
            class="bg-white w-16 h-16 flex items-center justify-center rounded-xl shadow-lg hover:shadow-2xl transition-shadow">
            <img :src="logo" alt="Logo" class="w-14 h-154 object-contain">
          </div>
        ...
        <nav class="hidden lg:flex items-center space-x-2">
          <router-link to="/"
            class="px-5 py-2.5 text-white hover:bg-white/20 rounded-lg transition-all font-semibold text-sm backdrop-blur-sm">
            Home
          </router-link>
        ...
        <div class="flex items-center space-x-3">
          <div class="hidden md:block relative">
            <input v-model="keyword" @keyup.enter="goToSearch" type="text" placeholder="Cari sparepart..."
          ...
          <template v-if="!isAuthenticated">
            <router-link to="/auth/login"
              class="hidden md:block px-5 py-2.5 text-black bg-[#FFD60A] backdrop-blur-sm border-2 border-[#FFD60A] rounded-lg hover:bg-white hover:text-[#BA181B] hover:border-white transition-all font-semibold text-sm shadow-lg">
              Masuk
            </router-link>

            <router-link to="/auth/register"
              class="hidden md:block px-5 py-2.5 text-white bg-white/10 backdrop-blur-sm border-2 border-white rounded-lg hover:bg-white hover:text-[#BA181B] transition-all font-semibold text-sm shadow-lg">
              Daftar
            </router-link>
          </template>
          ...
          <template v-else>
            ...
            <button @click="logout"
              class="px-5 py-2.5 text-white bg-white/10 backdrop-blur-sm border-2 border-white rounded-lg hover:bg-white hover:text-[#BA181B] transition-all font-semibold text-sm shadow-lg">
              Keluar
            </button>
          </template>

          <button @click="mobileMenuOpen = !mobileMenuOpen"
            class="lg:hidden text-white p-2.5 hover:bg-white/20 rounded-lg transition-all">
        ...
        <div class="max-w-7xl mx-auto px-4 py-4 space-y-2">
          <div class="relative mb-3">
            <input type="text" placeholder="Cari sparepart..."
              class="w-full px-4 py-2.5 pr-11 rounded-lg bg-[#0B090A] text-white placeholder-[#B1A7A6] border-2 border-[#660708] focus:border-[#E5383B] focus:outline-none transition-all" />
          ...
          <router-link to="/" @click="mobileMenuOpen = false"
            class="block px-4 py-3 text-white hover:bg-[#BA181B] rounded-lg transition-all font-semibold">
            Home
          </router-link>
          ...
          <router-link to="/request" @click="mobileMenuOpen = false"
            class="block px-4 py-3 text-white hover:bg-[#BA181B] rounded-lg transition-all font-semibold">
            Request
          </router-link>
          ...
          <template v-if="!isAuthenticated">
            <router-link to="/auth/login" @click="mobileMenuOpen = false"
              class="block text-center w-full px-4 py-3 text-white border-2 border-[#E5383B] rounded-lg hover:bg-[#E5383B] transition-all font-semibold mt-2">
              Masuk
            </router-link>

            <router-link to="/auth/register" @click="mobileMenuOpen = false"
              class="block text-center w-full px-4 py-3 text-[#161A1D] bg-[#FFD60A] rounded-lg hover:bg-yellow-300 transition-all font-semibold mt-2">
              Daftar
            </router-link>
          </template>
          ...
          <template v-else>
            ...
            <button @click="logout"
              class="w-full px-4 py-3 text-white border-2 border-[#E5383B] rounded-lg hover:bg-[#E5383B] transition-all font-semibold mt-2">
              Keluar
            </button>
          </template>
...
const cartCount = ref(0) // Ganti dengan logika sebenarnya untuk mendapatkan jumlah item di keranjang
